...
def move(
    ...
    return new_grid, new_limits, move_happened
...
round = 1
while move_happened:
    starting_direction = directions[(round - 1) % len(directions)]
    proposals: Dict[complex, int] = get_proposals(grid, grid_limits, starting_direction)
    # print_grid(proposals, grid_limits)
    grid, grid_limits, move_happened = move(
        grid, proposals, grid_limits, starting_direction
    )
    print(f"== End of Round {round} ==")
    # print_grid(grid, grid_limits)
    if not move_happened:
        print(f"No moves in round {round}")
    round += 1
