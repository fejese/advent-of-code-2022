...
# NOPE INPUT_FILE_NAME: str = "test-input"
    ...
    for i in range(steps):
        ...
        if curr_dir == Dir.L:
            if next_pos.x < 0 or grid[next_pos.y][next_pos.x] == " ":
                ...
                if 0 <= curr_pos.y < 50:
                    next_pos = C(0, 149 - curr_pos.y)
                    next_dir = Dir.R
                elif 50 <= curr_pos.y < 100:
                    next_pos = C(curr_pos.y - 50, 100)
                    next_dir = Dir.D
                elif 100 <= curr_pos.y < 150:
                    next_pos = C(50, 149 - curr_pos.y)
                    next_dir = Dir.R
                elif 150 <= curr_pos.y < 200:
                    next_pos = C(curr_pos.y - 100, 0)
                    next_dir = Dir.D
        elif curr_dir == Dir.R:
            if (
                ...
                if 0 <= curr_pos.y < 50:
                    next_pos = C(99, 149 - curr_pos.y)
                    next_dir = Dir.L
                elif 50 <= curr_pos.y < 100:
                    next_pos = C(curr_pos.y + 50, 49)
                    next_dir = Dir.U
                elif 100 <= curr_pos.y < 150:
                    next_pos = C(149, 149 - curr_pos.y)
                    next_dir = Dir.L
                elif 150 <= curr_pos.y < 200:
                    next_pos = C(curr_pos.y - 100, 149)
                    next_dir = Dir.U
        elif curr_dir == Dir.D:
            if next_pos.y > len(grid) - 1 or grid[next_pos.y][next_pos.x] == " ":
                # print("  wrapping around going down")
                if 0 <= curr_pos.x < 50:
                    next_pos = C(100 + curr_pos.x, 0)
                elif 50 <= curr_pos.x < 100:
                    next_pos = C(49, curr_pos.x + 100)
                    next_dir = Dir.L
                elif 100 <= curr_pos.x < 150:
                    next_pos = C(99, curr_pos.x - 50)
                    next_dir = Dir.L
        elif curr_dir == Dir.U:
            if next_pos.y < 0 or grid[next_pos.y][next_pos.x] == " ":
                # print("  wrapping around going up")
                if 0 <= curr_pos.x < 50:
                    next_pos = C(50, 50 + curr_pos.x)
                    next_dir = Dir.R
                elif 50 <= curr_pos.x < 100:
                    next_pos = C(0, curr_pos.x + 100)
                    next_dir = Dir.R
                elif 100 <= curr_pos.x < 150:
                    next_pos = C(curr_pos.x - 100, 199)
...
for instr in instructions:
    if instr.isnumeric():
        ...
        curr_pos, curr_dir = step(grid, curr_dir, curr_pos, steps)
    else:
        if instr == "L":
            print("Turning left")
